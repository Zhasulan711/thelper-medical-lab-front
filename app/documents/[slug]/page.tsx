...
const DOCUMENT_CONTENT: Record<string, { title: string; content: string }> = {
  privacy: {
    ...
    content: `
1. Общие положения
Настоящая политика определяет порядок обработки персональных данных посетителей сайта и пациентов лаборатории T-Helper.
Оператор персональных данных: ТОО «ОМИКРОН 3Д», БИН 191140005638, юридический адрес: 050000, г. Алматы, Ауэзовский район, мкр. Таугуль, ул. Аманжол, д. 24.

2. Сбор данных
Оператор может собирать следующие персональные данные: ФИО, контактный телефон, адрес электронной почты, а также медицинские данные в объёме, необходимом для оказания лабораторных услуг.
...
Вы имеете право на доступ к своим персональным данным, их уточнение, блокирование или уничтожение. Для этого направьте запрос на info@thelper.kz или обратитесь по телефону +7 (777) 276-40-40.
...
Оператор вправе вносить изменения в настоящую политику. Актуальная версия размещена на сайте.
  ...
  offer: {
    ...
    content: `
1. Термины
Исполнитель — ТОО «ОМИКРОН 3Д» (БИН 191140005638), оказывающее лабораторные услуги под брендом T-Helper.
Заказчик — физическое или юридическое лицо, принявшее условия оферты.
...
ТОО «ОМИКРОН 3Д»
БИН: 191140005638
Юридический адрес: 050000, г. Алматы, Ауэзовский район, мкр. Таугуль, ул. Аманжол, д. 24
Контакты: info@thelper.kz, +7 (777) 276-40-40
WhatsApp: https://wa.me/77772764040
    `.trim(),
  },
}
